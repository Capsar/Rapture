...
public class ItemEnderEye extends Item
{
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        if (playerIn.canPlayerEdit(pos.offset(side), side, stack) && var9.getBlock() == Blocks.end_portal_frame && !((Boolean)var9.getValue(BlockEndPortalFrame.EYE)).booleanValue())
        {
            if (worldIn.isRemote)
            {
                return true;
            ...
            {
                worldIn.setBlockState(pos, var9.withProperty(BlockEndPortalFrame.EYE, Boolean.valueOf(true)), 2);
                ...
                if (var14 && var12 == var24 + 2)
                {
                    ...
                    for (var29 = var24; var29 <= var12; ++var29)
                    {
                        ...
                        if (var32.getBlock() != Blocks.end_portal_frame || !((Boolean)var32.getValue(BlockEndPortalFrame.EYE)).booleanValue())
                        {
                            var14 = false;
                            break;
                        }
    ...
    public ItemStack onItemRightClick(ItemStack itemStackIn, World worldIn, EntityPlayer playerIn)
    {
        ...
        else
        {
            if (!worldIn.isRemote)
            {
                BlockPos var5 = worldIn.getStrongholdPos("Stronghold", new BlockPos(playerIn));
