...
public class ItemInWorldManager
{
    ...
    public void blockRemoving(BlockPos pos)
    {
        if (pos.equals(this.field_180240_f))
        {
            ...
            if (var3.getMaterial() != Material.air)
            {
                float var4 = var3.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.worldObj, pos) * (float)(var2 + 1);
    ...
    public boolean activateBlockOrUseItem(EntityPlayer player, World worldIn, ItemStack stack, BlockPos pos, EnumFacing side, float offsetX, float offsetY, float offsetZ)
    {
        ...
        else
        {
            if (!player.isSneaking() || player.getHeldItem() == null)
            {
                ...
                if (var9.getBlock().onBlockActivated(worldIn, pos, var9, player, side, offsetX, offsetY, offsetZ))
                {
                    return true;
                }
            }
            ...
            else if (this.isCreative())
            {
                ...
                boolean var11 = stack.onItemUse(player, worldIn, pos, side, offsetX, offsetY, offsetZ);
