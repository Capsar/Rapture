...
public class WorldGenMegaPineTree extends WorldGenHugeTrees
{
	...
	public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        {
            this.func_150541_c(worldIn, position.getX(), position.getZ(), position.getY() + var4, 0, rand);

            for (int var5 = 0; var5 < var4; ++var5)
            {
                Block var6 = worldIn.getBlockState(position.up(var5)).getBlock();

                if (var6.getMaterial() == Material.air || var6.getMaterial() == Material.leaves)
                {
                    this.setBlock(worldIn, position.up(var5), Blocks.log, this.woodMetadata);
                }

                if (var5 < var4 - 1)
                {
                    var6 = worldIn.getBlockState(position.add(1, var5, 0)).getBlock();

                    if (var6.getMaterial() == Material.air || var6.getMaterial() == Material.leaves)
                    {
                        this.setBlock(worldIn, position.add(1, var5, 0), Blocks.log, this.woodMetadata);
                    }

                    var6 = worldIn.getBlockState(position.add(1, var5, 1)).getBlock();

                    if (var6.getMaterial() == Material.air || var6.getMaterial() == Material.leaves)
                    {
                        this.setBlock(worldIn, position.add(1, var5, 1), Blocks.log, this.woodMetadata);
                    }

                    var6 = worldIn.getBlockState(position.add(0, var5, 1)).getBlock();

                    if (var6.getMaterial() == Material.air || var6.getMaterial() == Material.leaves)
                    {
                        this.setBlock(worldIn, position.add(0, var5, 1), Blocks.log, this.woodMetadata);
                    }
                }
            }
    ...
    private void func_175934_c(World worldIn, BlockPos p_175934_2_)
    {
        for (int var3 = 2; var3 >= -3; --var3)
        {
            ...
            {
                this.setBlock(worldIn, var4, Blocks.dirt, BlockDirt.DirtType.PODZOL.getMetadata());
                break;
            }
