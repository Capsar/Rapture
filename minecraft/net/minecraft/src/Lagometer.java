    ...
    public static void showLagometer(ScaledResolution scaledResolution)
    {
        if (gameSettings != null && gameSettings.ofLagometer)
        {
            ...
            for (int lumMem = 0; lumMem < timesFrame.length; ++lumMem)
            {
                ...
                memColR += 155;
                float memColG = mc.displayHeight;
                long memColB = 0L;
                ...
                else
                {
                    ...
                    memColG -= renderTime(lumMem, timesServer[lumMem], memColR / 2, memColR / 2, memColR / 2, memColG, tessellator);
                    memColG -= renderTime(lumMem, timesTerrain[lumMem], 0, memColR, 0, memColG, tessellator);
                    memColG -= renderTime(lumMem, timesVisibility[lumMem], memColR, memColR, 0, memColG, tessellator);
                    memColG -= renderTime(lumMem, timesChunkUpdate[lumMem], memColR, 0, 0, memColG, tessellator);
                    memColG -= renderTime(lumMem, timesChunkUpload[lumMem], memColR, 0, memColR, memColG, tessellator);
                    memColG -= renderTime(lumMem, timesScheduledExecutables[lumMem], 0, 0, memColR, memColG, tessellator);
                    float var10000 = memColG - renderTime(lumMem, timesTick[lumMem], 0, memColR, memColR, memColG, tessellator);
                }
            }
