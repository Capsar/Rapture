...
public class ConnectedTextures
{
    ...
    {
        int[] data = (int[])quad.getVertexData().clone();
        ...
        BakedQuad var5 = new BakedQuad(data, quad.getTintIndex(), quad.getFace(), sprite);
        return var5;
    }
    ...
    public static int getSide(EnumFacing facing)
    {
        ...
        {
            switch (ConnectedTextures.NamelessClass87119709.$SwitchMap$net$minecraft$util$EnumFacing[facing.ordinal()])
            {
                case 1:
                    return 0;
    ...
    private static TextureAtlasSprite getConnectedTexture(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv)
    {
        ...
        if (y >= cp.minHeight && y <= cp.maxHeight)
        {
            if (cp.biomes != null)
            {
                ...
                for (int block = 0; block < cp.biomes.length; ++block)
                {
                    BiomeGenBase mds = cp.biomes[block];
    ...
    private static TextureAtlasSprite getConnectedTextureCtm(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, int side, TextureAtlasSprite icon, int metadata, RenderEnv renderEnv)
    {
        ...
        switch (side)
        {
            ...
            case 2:
                ...
                borders[2] = isNeighbour(cp, blockAccess, blockState, blockPos.down(), side, icon, metadata);
                borders[3] = isNeighbour(cp, blockAccess, blockState, blockPos.up(), side, icon, metadata);
                break;
        ...
        else
        {
            ...
            {
                case 0:
                    borders[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().north(), side, icon, metadata);
                    borders[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().north(), side, icon, metadata);
                    borders[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().south(), side, icon, metadata);
                    borders[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().south(), side, icon, metadata);
                    break;

                case 1:
                    borders[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().south(), side, icon, metadata);
                    borders[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().south(), side, icon, metadata);
                    borders[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().north(), side, icon, metadata);
                    borders[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().north(), side, icon, metadata);
                    break;

                case 2:
                    borders[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().down(), side, icon, metadata);
                    borders[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().down(), side, icon, metadata);
                    borders[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().up(), side, icon, metadata);
                    borders[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().up(), side, icon, metadata);
                    break;

                case 3:
                    borders[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().down(), side, icon, metadata);
                    borders[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().down(), side, icon, metadata);
                    borders[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.east().up(), side, icon, metadata);
                    borders[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.west().up(), side, icon, metadata);
                    break;

                case 4:
                    borders[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.down().south(), side, icon, metadata);
                    borders[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.down().north(), side, icon, metadata);
                    borders[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.up().south(), side, icon, metadata);
                    borders[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.up().north(), side, icon, metadata);
                    break;

                case 5:
                    borders[0] = !isNeighbour(cp, blockAccess, blockState, blockPos.down().north(), side, icon, metadata);
                    borders[1] = !isNeighbour(cp, blockAccess, blockState, blockPos.down().south(), side, icon, metadata);
                    borders[2] = !isNeighbour(cp, blockAccess, blockState, blockPos.up().north(), side, icon, metadata);
                    borders[3] = !isNeighbour(cp, blockAccess, blockState, blockPos.up().south(), side, icon, metadata);
            }
    ...
    private static TextureAtlasSprite getNeighbourIcon(IBlockAccess iblockaccess, BlockPos blockPos, IBlockState neighbourState, int side)
    {
        ...
        IBakedModel model = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getModelForState(neighbourState);
        ...
        else
        {
            ...
            List quads = model.getFaceQuads(facing);
            ...
            {
                List quadsGeneral = model.getGeneralQuads();
        ...
        switch (vertAxis)
        {
            ...
            case 1:
                switch (side)
                {
                    ...
                    case 4:
                        left = isNeighbour(cp, blockAccess, blockState, blockPos.down(), side, icon, metadata);
                        right = isNeighbour(cp, blockAccess, blockState, blockPos.up(), side, icon, metadata);
                        break label46;

                    case 5:
                        left = isNeighbour(cp, blockAccess, blockState, blockPos.up(), side, icon, metadata);
                        right = isNeighbour(cp, blockAccess, blockState, blockPos.down(), side, icon, metadata);
    ...
    private static TextureAtlasSprite getConnectedTextureVertical(ConnectedProperties cp, IBlockAccess blockAccess, IBlockState blockState, BlockPos blockPos, int vertAxis, int side, TextureAtlasSprite icon, int metadata)
    {
        ...
        switch (vertAxis)
        {
            case 0:
                ...
                bottom = isNeighbour(cp, blockAccess, blockState, blockPos.down(), side, icon, metadata);
                top = isNeighbour(cp, blockAccess, blockState, blockPos.up(), side, icon, metadata);
                break;
    ...
    public static void updateIcons(TextureMap textureMap)
    {
        ...
        if (Config.isConnectedTextures())
        {
            ...
            emptySprite = textureMap.registerSprite(var4);
    ...
    static class NamelessClass87119709
    {
