...
public class ConnectedProperties
{
    ...
    public ConnectedProperties(Properties props, String path)
    {
        ...
        String propMatchBlocks = props.getProperty("matchBlocks");
        IBlockState bs = this.parseBlockState(propMatchBlocks);

        if (bs != null)
        {
            this.matchBlocks = new int[] {Block.getIdFromBlock(bs.getBlock())};
            this.metadatas = new int[] {bs.getBlock().getMetaFromState(bs)};
        }

        if (this.matchBlocks == null)
        {
            this.matchBlocks = parseBlockIds(propMatchBlocks);
        }
    ...
    private IBlockState parseBlockState(String propMatchBlocks)
    {
        ...
        {
            String[] parts = Config.tokenize(propMatchBlocks, ":");
                ...
                else
                {
                    ...
                    for (int i = 2; i < parts.length; ++i)
                    {
                        ...
                        if (part.length() >= 1)
                        {
                            ...
                            {
                                if (part.indexOf(45) < 0 && part.indexOf(44) < 0)
                                {
                                    int tokens = Config.parseInt(part, -1);
                            ...
                            {
                                String[] var19 = Config.tokenize(part, "=");

                                if (var19.length >= 2)
                                {
                                    String key = var19[0];
                                    String value = var19[1];
                                    ...
                                    if (value.indexOf(44) < 0)
                                    {
                                        ...
                                        Collection properties = state.getPropertyNames();
                                        IProperty prop = this.getProperty(key, properties);

                                        if (prop == null)
                                        {
                                            String valueClass = "\"";
                                            Config.warn("Block " + valueClass + blockName + valueClass + " has no property " + valueClass + key + valueClass);
                                        }
                                        else
                                        {
                                            Class var20 = prop.getValueClass();
                                            Object valueObj = this.parseValue(value, var20);

                                            if (valueObj == null)
                                            {
                                                Collection valueObjComp = prop.getAllowedValues();
                                                valueObj = this.getPropertyValue(value, valueObjComp);
                                            }

                                            if (valueObj == null)
                                            {
                                                Config.warn("Invalid value: " + value + ", for property: " + prop);
                                            }
                                            else if (!(valueObj instanceof Comparable))
                                            {
                                                Config.warn("Value is not Comparable: " + value + ", for property: " + prop);
                                            }
                                            else
                                            {
                                                Comparable var21 = (Comparable)valueObj;
                                                state = state.withProperty(prop, var21);
                                            }
                                        }
                                    }
                                }
    ...
    private Object getPropertyValue(String value, Collection propertyValues)
    {
    ...
    private IProperty getProperty(String key, Collection properties)
    {
