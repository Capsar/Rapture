...
public class CustomColorizer
{
        ...
        for (r = x - 1; r <= x + 1; ++r)
        {
            for (g = z - 1; g <= z + 1; ++g)
            {
                ...
                if (b == null)
                {
                    ...
                    {
                        case 1:
                            var20 = blockAccess.getBiomeGenForCoords(posM).getGrassColorAtPos(posM);
                            break;
                        ...
                        case 2:
                            ...
                            {
                                var20 = blockAccess.getBiomeGenForCoords(posM).getFoliageColorAtPos(posM);
                            }
    ...
    {
        return block.getMaterial() != Material.water ? block.colorMultiplier(blockAccess, blockPos) : (waterColors != null ? (Config.isSmoothBiomes() ? getSmoothColor(waterColors, blockAccess, blockPos.getX(), blockPos.getY(), blockPos.getZ(), 3, 1) : getCustomColor(waterColors, blockAccess, blockPos)) : (!Config.isSwampColors() ? 16777215 : block.colorMultiplier(blockAccess, blockPos)));
    }
    ...
    private static int getCustomColor(int[] colors, IBlockAccess blockAccess, BlockPos blockPos)
    {
        ...
        double temperature = MathHelper.clamp_float(bgb.getFloatTemperature(blockPos), 0.0F, 1.0F);
        ...
        else
        {
            ...
            for (r = x0 - n; r <= x0 + n; r += step)
            {
                for (g = z0 - n; g <= z0 + n; g += step)
                {
                    ...
                    b = getCustomColor(colors, blockAccess, blockPosM);
                    sumRed += b >> 16 & 255;
                    sumGreen += b >> 8 & 255;
                    sumBlue += b & 255;
                    ++count;
