...
public class EntityWitherSkull extends EntityFireball
{
    ...
    {
        float var5 = super.getExplosionResistance(explosionIn, worldIn, pos, blockStateIn);

        if (this.isInvulnerable() && blockStateIn.getBlock() != Blocks.bedrock && blockStateIn.getBlock() != Blocks.end_portal && blockStateIn.getBlock() != Blocks.end_portal_frame && blockStateIn.getBlock() != Blocks.command_block)
        {
        ...
        if (!this.worldObj.isRemote)
        {
            if (movingObject.entityHit != null)
            {
                ...
                if (movingObject.entityHit instanceof EntityLivingBase)
                {
                    ...
                    if (var2 > 0)
                    {
                        ((EntityLivingBase)movingObject.entityHit).addPotionEffect(new PotionEffect(Potion.wither.id, 20 * var2, 1));
                    }
                }
            }
