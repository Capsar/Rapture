...
public abstract class EntityPlayer extends EntityLivingBase {
		...
		if (itemStackIn.getItemUseAction() == EnumAction.EAT) {
			for (int var3 = 0; var3 < p_71010_2_; ++var3) {
				...
				if (itemStackIn.getHasSubtypes()) {
					this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, var7.xCoord, var7.yCoord, var7.zCoord, var4.xCoord,
							var4.yCoord + 0.05D, var4.zCoord,
							new int[] { Item.getIdFromItem(itemStackIn.getItem()), itemStackIn.getMetadata() });
				} else {
					this.worldObj.spawnParticle(EnumParticleTypes.ITEM_CRACK, var7.xCoord, var7.yCoord, var7.zCoord, var4.xCoord,
							var4.yCoord + 0.05D, var4.zCoord, new int[] { Item.getIdFromItem(itemStackIn.getItem()) });
				}
			}
	...
	private Collection func_175137_e(Entity p_175137_1_) {
		ScorePlayerTeam var2 = this.getWorldScoreboard().getPlayersTeam(this.getCommandSenderName());

		if (var2 != null) {
			int var3 = var2.getChatFormat().getColorIndex();

			if (var3 >= 0 && var3 < IScoreObjectiveCriteria.field_178793_i.length) {
				Iterator var4 = this.getWorldScoreboard().getObjectivesFromCriteria(IScoreObjectiveCriteria.field_178793_i[var3])
						.iterator();

				while (var4.hasNext()) {
					ScoreObjective var5 = (ScoreObjective) var4.next();
					Score var6 = this.getWorldScoreboard().getValueFromObjective(p_175137_1_.getCommandSenderName(), var5);
					var6.func_96648_a();
				}
			}
		}

		ScorePlayerTeam var7 = this.getWorldScoreboard().getPlayersTeam(p_175137_1_.getCommandSenderName());
	...
	public EntityItem dropOneItem(boolean dropAll) {
		return this.dropItem(this.inventory.decrStackSize(this.inventory.currentItem,
				dropAll && this.inventory.getCurrentItem() != null ? this.inventory.getCurrentItem().stackSize : 1), false, true);
	}
	...
	public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
		if (droppedItem == null) {
			...
			EntityItem var6 = new EntityItem(this.worldObj, this.posX, var4, this.posZ, droppedItem);
			var6.setPickupDelay(40);

			if (traceItem) {
				var6.setThrower(this.getCommandSenderName());
			}
			...
			if (dropAround) {
				...
				var7 = 0.3F;
				var6.motionX = (double) (-MathHelper.sin(this.rotationYaw / 180.0F * (float) Math.PI)
						* MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * var7);
				var6.motionZ = (double) (MathHelper.cos(this.rotationYaw / 180.0F * (float) Math.PI)
						* MathHelper.cos(this.rotationPitch / 180.0F * (float) Math.PI) * var7);
	...
	public boolean canHarvestBlock(Block blockToHarvest) {
		return this.inventory.func_146025_b(blockToHarvest);
	}
	...
	public void readEntityFromNBT(NBTTagCompound tagCompund) {
		...
		if (tagCompund.hasKey("SpawnX", 99) && tagCompund.hasKey("SpawnY", 99) && tagCompund.hasKey("SpawnZ", 99)) {
			this.spawnChunk = new BlockPos(tagCompund.getInteger("SpawnX"), tagCompund.getInteger("SpawnY"),
					...
					if (var14) {
						if (var18 > 0) {
							targetEntity.addVelocity(
									(double) (-MathHelper.sin(this.rotationYaw * (float) Math.PI / 180.0F) * (float) var18 * 0.5F), 0.1D,
									(double) (MathHelper.cos(this.rotationYaw * (float) Math.PI / 180.0F) * (float) var18 * 0.5F));
							this.motionX *= 0.6D;
							this.motionZ *= 0.6D;
	...
	public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation) {
		if (!this.worldObj.isRemote) {
			...
			if (Math.abs(this.posX - (double) bedLocation.getX()) > 3.0D || Math.abs(this.posY - (double) bedLocation.getY()) > 2.0D
					|| Math.abs(this.posZ - (double) bedLocation.getZ()) > 3.0D) {
				return EntityPlayer.EnumStatus.TOO_FAR_AWAY;
			}

			double var2 = 8.0D;
			double var4 = 5.0D;
			List var6 = this.worldObj.getEntitiesWithinAABB(EntityMob.class, new AxisAlignedBB((double) bedLocation.getX() - var2,
					(double) bedLocation.getY() - var4, (double) bedLocation.getZ() - var2, (double) bedLocation.getX() + var2,
					(double) bedLocation.getY() + var4, (double) bedLocation.getZ() + var2));
		...
		if (this.worldObj.isBlockLoaded(bedLocation)) {
			EnumFacing var7 = (EnumFacing) this.worldObj.getBlockState(bedLocation).getValue(BlockDirectional.FACING);
			float var3 = 0.5F;
			float var8 = 0.5F;

			switch (EntityPlayer.SwitchEnumFacing.FACING_LOOKUP[var7.ordinal()]) {
			case 1:
				var8 = 0.9F;
				break;
			...
			this.setPosition((double) ((float) bedLocation.getX() + var3), (double) ((float) bedLocation.getY() + 0.6875F),
					(double) ((float) bedLocation.getZ() + var8));
		} else {
			this.setPosition((double) ((float) bedLocation.getX() + 0.5F), (double) ((float) bedLocation.getY() + 0.6875F),
	...
	public void wakeUpPlayer(boolean p_70999_1_, boolean updateWorldFlag, boolean setSpawn) {
		...
		if (this.playerLocation != null && var4.getBlock() == Blocks.bed) {
			this.worldObj.setBlockState(this.playerLocation, var4.withProperty(BlockBed.OCCUPIED, Boolean.valueOf(false)), 4);
			...
			this.setPosition((double) ((float) var5.getX() + 0.5F), (double) ((float) var5.getY() + 0.1F),
					(double) ((float) var5.getZ() + 0.5F));
		}
	 ...
	 * 
	 * @param forceSpawn
	 *            Spawn at bed location even if bed is missing.
	 */
	public static BlockPos getBedSpawnLocation(World worldIn, BlockPos bedLocation, boolean forceSpawn) {
		if (worldIn.getBlockState(bedLocation).getBlock() != Blocks.bed) {
			if (!forceSpawn) {
				...
				Material var3 = worldIn.getBlockState(bedLocation).getBlock().getMaterial();
				Material var4 = worldIn.getBlockState(bedLocation.up()).getBlock().getMaterial();
				boolean var5 = !var3.isSolid() && !var3.isLiquid();
				boolean var6 = !var4.isSolid() && !var4.isLiquid();
				return var5 && var6 ? bedLocation : null;
			}
		} else {
	...
	public float getBedOrientationInDegrees() {
		if (this.playerLocation != null) {
			...
			switch (EntityPlayer.SwitchEnumFacing.FACING_LOOKUP[var1.ordinal()]) {
			case 1:
				return 90.0F;
	...
	public void addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
		if (this.ridingEntity == null) {
			...
			if (this.isInsideOfMaterial(Material.water)) {
				var7 = Math
						.round(MathHelper.sqrt_double(p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_) * 100.0F);
	...
	private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
		if (this.ridingEntity != null) {
			int var7 = Math
					.round(MathHelper.sqrt_double(p_71015_1_ * p_71015_1_ + p_71015_3_ * p_71015_3_ + p_71015_5_ * p_71015_5_) * 100.0F);
			...
			if (var7 > 0) {
				if (this.ridingEntity instanceof EntityMinecart) {
					...
					} else if (this.startMinecartRidingCoordinate.distanceSq((double) MathHelper.floor_double(this.posX),
							(double) MathHelper.floor_double(this.posY), (double) MathHelper.floor_double(this.posZ)) >= 1000000.0D) {
	...
	public void onKillEntity(EntityLivingBase entityLivingIn) {
		...
		EntityList.EntityEggInfo var2 = (EntityList.EntityEggInfo) EntityList.entityEggs.get(Integer.valueOf(EntityList
				.getEntityID(entityLivingIn)));
	...
	public IChatComponent getDisplayName() {
		...
		var1.getChatStyle().setInsertion(this.getCommandSenderName());
		return var1;
	}
	...
	public static UUID getUUID(GameProfile profile) {
		...
		if (var1 == null) {
			var1 = getOfflineUUID(profile.getName());
		}
	...
	public boolean canOpen(LockCode code) {
		if (code.isEmpty()) {
			...
			ItemStack var2 = this.getCurrentEquippedItem();
			return var2 != null && var2.hasDisplayName() ? var2.getDisplayName().equals(code.getLock()) : false;
		}
	}
			...
			if (var3 >= 0 && var3 < this.inventory.armorInventory.length) {
				...
				if (itemStackIn != null && itemStackIn.getItem() != null) {
					if (itemStackIn.getItem() instanceof ItemArmor) {
						if (EntityLiving.getArmorPosition(itemStackIn) != var4) {
							return false;
						}
					} else if (var4 != 4 || itemStackIn.getItem() != Items.skull && !(itemStackIn.getItem() instanceof ItemBlock)) {
						return false;
					}
				}

				this.inventory.setInventorySlotContents(var3 + this.inventory.mainInventory.length, itemStackIn);
				return true;
			} else {
				var4 = inventorySlot - 200;

				if (var4 >= 0 && var4 < this.theInventoryEnderChest.getSizeInventory()) {
					this.theInventoryEnderChest.setInventorySlotContents(var4, itemStackIn);
					return true;
				} else {
					return false;
				}
	...
	public static enum EnumChatVisibility {
		FULL("FULL", 0, 0, "options.chat.visibility.full"), SYSTEM("SYSTEM", 1, 1, "options.chat.visibility.system"), HIDDEN("HIDDEN", 2,
				2, "options.chat.visibility.hidden");
		...
		private EnumChatVisibility(String p_i45323_1_, int p_i45323_2_, int id, String resourceKey) {
	...
	public static enum EnumStatus {
		OK("OK", 0), NOT_POSSIBLE_HERE("NOT_POSSIBLE_HERE", 1), NOT_POSSIBLE_NOW("NOT_POSSIBLE_NOW", 2), TOO_FAR_AWAY("TOO_FAR_AWAY", 3), OTHER_PROBLEM(
				"OTHER_PROBLEM", 4), NOT_SAFE("NOT_SAFE", 5);

		private static final EntityPlayer.EnumStatus[] $VALUES = new EntityPlayer.EnumStatus[] { OK, NOT_POSSIBLE_HERE, NOT_POSSIBLE_NOW,
				TOO_FAR_AWAY, OTHER_PROBLEM, NOT_SAFE };
