...
public abstract class EntityMob extends EntityCreature implements IMob
{
    ...
    public boolean attackEntityAsMob(Entity entityIn)
    {
        ...
        if (var4)
        {
            if (var3 > 0)
            {
                entityIn.addVelocity((double)(-MathHelper.sin(this.rotationYaw * (float)Math.PI / 180.0F) * (float)var3 * 0.5F), 0.1D, (double)(MathHelper.cos(this.rotationYaw * (float)Math.PI / 180.0F) * (float)var3 * 0.5F));
                this.motionX *= 0.6D;
                this.motionZ *= 0.6D;
            }
