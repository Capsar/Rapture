...
public class EnchantmentThorns extends Enchantment
{
    ...
    {
        Random var4 = user.getRNG();
        ItemStack var5 = EnchantmentHelper.getEnchantedItem(Enchantment.thorns, user);

        if (func_92094_a(level, var4))
        {
            attacker.attackEntityFrom(DamageSource.causeThornsDamage(user), (float)func_92095_b(level, var4));
            attacker.playSound("damage.thorns", 0.5F, 1.0F);

            if (var5 != null)
            {
                var5.damageItem(3, user);
            }
        }
        else if (var5 != null)
        {
            var5.damageItem(1, user);
        }
    }
