...
public class GuiPasswordField extends Gui {
	...
	public void func_50031_b(String par1Str) {
		...
		if (k < par1Str.length()) {
			s = (new StringBuilder()).append(s).append(par1Str.substring(0, k)).toString();
			l = k;
		} else {
			s = (new StringBuilder()).append(s).append(par1Str).toString();
			l = par1Str.length();
		}
	...
	public boolean textboxKeyTyped(char par1, int par2) {
		if (!field_50043_m || !isFocused) { return false; }
	...
	public void drawTextBox() {
		...
		if (s.length() > 0) {
			...
			j1 = FontUtil.drawScaledString(text.replaceAll(".", "*"), j1, i1, i, 1, true);
		}
		...
		if (s.length() > 0 && flag && j < s.length()) {
			j1 = FontUtil.drawScaledString(s.substring(j), j1, i1, i, 1, true);
		}
		...
		if (flag1) {
			if (flag2) {
				...
				FontUtil.drawScaledString("_", k1, i1, i, 1, true);
			}
		}
	...
	private void func_50029_c(int par1, int par2, int par3, int par4) {
		...
		Wrapper.getWorldRenderer().addVertex(par1, par4, 0.0D);
		Wrapper.getWorldRenderer().addVertex(par3, par4, 0.0D);
		Wrapper.getWorldRenderer().addVertex(par3, par2, 0.0D);
		Wrapper.getWorldRenderer().addVertex(par1, par2, 0.0D);
		Wrapper.getWorldRenderer().drawMultiTexture();
	...
	public void func_50032_g(int par1) {
		...
		if (fontRenderer != null) {
			String s = fontRenderer.trimStringToWidth(text.substring(field_50041_n), func_50019_l());
			int j = s.length() + field_50041_n;

			if (par1 > j) {
				field_50041_n += par1 - j;
