...
public class RaptureFont {
	...
	public RaptureFont(final Font font, final boolean antiAlias, final int charOffset) {
		this.font = font;
	...
	public RaptureFont(final Font font, final boolean antiAlias) {
		this.font = font;
		this.antiAlias = antiAlias;
		charOffset = 8;
		setupTexture(antiAlias);
	...
	private void drawQuad(final float x, final float y, final float width, final float height, final float srcX, final float srcY,
			final float srcWidth, final float srcHeight) {
		final float renderSRCX = srcX / IMAGE_WIDTH, renderSRCY = srcY / IMAGE_HEIGHT, renderSRCWidth = (srcWidth) / IMAGE_WIDTH, renderSRCHeight = (srcHeight)
				/ IMAGE_HEIGHT;
	...
	public int getStringWidth(final String text) {
		...
		for(final char c : text.toCharArray())
			if ((c < chars.length) && (c >= 0)) width += chars[c].width - charOffset;
		return width / 2;
	}
